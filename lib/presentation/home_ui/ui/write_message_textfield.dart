...
import 'package:bytebrain_project_akhir/core/provider/speech_to_text_provider.dart';
import 'package:bytebrain_project_akhir/presentation/home_ui/costum_snackbar/snack_bar.dart';
    ...
    if (_isVoiceChat) {
      ref.read(speechToTextProvider).isAvailabe().then((available) {
        if (available) {
          ...
          showSnackBarMessage(context, 'Speech recognition tidak tersedia');
        }
      });
    } else {
  ...
  void _sendPromptProvider(String prompt) async {
    ...
    try {
      ...
      showSnackBarMessage(context,
          'Terjadi kesalahan! coba restart aplikasi, tanya pertanyaan lain, atau anda sudah melebihi batas pemakaian harian.');
    }
    setState(() {
      _isLoading = false;
    });
